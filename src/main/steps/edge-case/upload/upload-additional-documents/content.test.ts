...
resourceLoader.Loader('upload-additional-documents');
...
describe('Upload content', () => {
  ...
  test('should generate uploadedDocuments for template', () => {
    ...
    expect(generatedContent.fileUploadConfig.uploadedFiles).toStrictEqual([
      { filename: 'file1', fileremoveUrl: '/upload/upload-additional-documents/123?' },
      { filename: 'file2', fileremoveUrl: '/upload/upload-additional-documents/1234?' },
    ]);
  });
});
